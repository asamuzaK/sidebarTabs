...
"use strict";
{
  ...
  const bookmarkTabGroup = async container => {
    ...
    if (container && container.nodeType === Node.ELEMENT_NODE) {
      ...
      if (classList.contains(CLASS_TAB_GROUP)) {
        const items =
          container.querySelectorAll(`${TAB_QUERY}:not(.${PINNED})`);
        for (const item of items) {
          const itemTab = item.dataset && item.dataset.tab &&
                            JSON.parse(item.dataset.tab);
          const {title, url} = itemTab;
  ...
  /**
   * undo close tab
   * @returns {?AsyncFunction} - restoreClosedTab()
   */
  const undoCloseTab = async () => {
    ...
    if (lastClosedTab) {
      ...
      if (isString(sessionId)) {
        func = restoreClosedTab(sessionId);
      }
    }
    return func || null;
  };
  ...
  const groupSelectedTabs = async () => {
    ...
    for (const item of items) {
      ...
      const {tabId: itemTabId} = itemDataset;
      if (Number.isInteger(firstTabId)) {
        itemDataset.group = true;
        arr.push(itemTabId * 1);
      } else {
        firstTabId = itemTabId * 1;
      }
    }
    if (Number.isInteger(firstTabId) && arr.length) {
      const {windowId} = sidebar;
      const opt = {
        windowId,
        index: -1,
      };
      await moveTab(firstTabId, opt);
   ...
   * @param {Object} elm - element
   * @param {string} placeholders - placeholders for localization
   * @returns {void}
   */
    ...
    if (elm && elm.nodeType === Node.ELEMENT_NODE && elm.hasAttributes()) {
      ...
      for (const item of items) {
        ...
        if (elm.hasAttribute(value)) {
          if (isString(placeholders) && placeholders.length) {
            elm.setAttribute(
              value,
